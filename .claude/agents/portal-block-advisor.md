...
tools: Read, Grep, Glob, mcp__zuar-portal__get_version, mcp__zuar-portal__get_block, mcp__zuar-portal__list_blocks, mcp__zuar-portal__list_resource, mcp__zuar-portal__get_resource, mcp__zuar-portal__execute_query, mcp__zuar-portal__profile_datasource, mcp__zuar-portal__active_config, mcp__claude-in-chrome__tabs_context_mcp, mcp__claude-in-chrome__tabs_create_mcp, mcp__claude-in-chrome__navigate, mcp__claude-in-chrome__computer, mcp__claude-in-chrome__read_page
---
...
6. **Does it read at a glance? (when you have eyes).** When `active_config` reports `config.browser.claudeInChrome`, the Claude for Chrome tools are connected, and the block is on a viewable page, **look at it** (read-only): does the headline land in the first second for the intended altitude, or does the eye have to hunt? Is the so-what obvious, the label legible, the chart honest at a glance? This is taste-level evidence the code can't give you — a screenshot tells you in one look whether an exec actually gets the answer. Skip gracefully (and judge from the structure) when it isn't viewable. See `zportal://guide/visual-verification`.
